...
void print_ingredients(intNode *h)
{
     ...
     * Complete this function
	 You have a linked list of integers where each integer represents an index in the ingredient array
     *****/
...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
	...
	int row = ingredient_index(source_ingredient);
	
	// check that source_ingredient is in AdjMat
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
	...
	for (int col = 0; col < MAT_SIZE; col++)
	{
		// if the ingredient is a neighbor 
		if (AdjMat[row][col] > 0)
		{
			// if it is not already in the linked list
			// insert it into the linked list 
			if (searchInt(h, col) != 1) // if it is not already in the list
			{
				h = insertInt(h, col);
			}
			// and then get its neighbors
...
intNode *deleteIngredient(intNode *head, int key)
{	
	// list is empty
	if (head == NULL)
	{
...
intNode *deleteAvoids(intNode *source_head, intNode *delete_head)
{
	// this function deletes all of the avoid ingredients from the source head list
	intNode *current = NULL;
	current = delete_head;
	
	while (current != NULL)
	{
		source_head = deleteIngredient(source_head, current->x);
		current = current->next;
	}
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
	...
	/*
		1. Add all the ingredients related to source_ingredient 
		2. Navigate the graph to find the restricted ingredients of at most k_avoid distance 
		3. Compile these restricted ingredients into a linked list
		4. Delete these ingredients from the linked list compiled in 1
	*/
	
	intNode *source_head = NULL;
	intNode *avoid_head = NULL;
	int delete_index = ingredient_index(avoid);
	
	// step 1: all ingredients
	source_head = related_k_dist(source_head, source_ingredient, k_source, 0);
	
	// step 2 and 3: avoid ingredients
	avoid_head = related_k_dist(avoid_head, avoid, k_avoid, 0);

	// step 4: delete the avoid ingredients from source list
	source_head = deleteAvoids(source_head, avoid_head);
	source_head = deleteIngredient(source_head, delete_index);
	
	// NO ONE LIKES MEMORY LEAKS!!!!
	deleteList(avoid_head);
	
	return source_head;
}

int get_index_of_max(int weights[MAT_SIZE])
{
	int max = INVALID_WEIGHT;
...
void remove_weights(int weights[MAT_SIZE], char recipe[10][MAX_STR_LEN])
{
	// this function removes the weights of ingredients already in the recipe
	int ing_index = 0;
	for (int i = 0; i < 10; i++)
	{
		ing_index = ingredient_index(recipe[i]);
		
		if (ing_index != NOT_FOUND)
		{
			weights[ing_index] = INVALID_WEIGHT;
		}
	}
	return;
}

int get_index_of_to_change_in_recipe(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
	for (int i = 0; i < 10; i++)
	{
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   * ingredient given the graph that is *not already in the recipe*.
	...
	change the ingredient with a substitute that is not already in the recipe
   * 
   ...
   */
    ...
    ******/
	...
	/*
	1. For each row (for each ingredient), get the weight of each column and keep track of each column
	2. keep summing up each column
	3. return the column number that has the biggest sum
	*/

	int change_index = get_index_of_to_change_in_recipe(recipe, to_change);

	
	if (change_index == NOT_FOUND)
	{
		return;
	}
	
	// each index represents an ingredient in the ingredients list 
	int weights[MAT_SIZE] = {0};
	int ing_row = 0; 
	// variables used for changing
	int max_index = 0;
	int change_ingredient_index = ingredient_index(to_change);
	
	// getting the weights of every other connection to the remaining ingredients
	for (int counter = 0; counter < 10; counter++)
	{
		ing_row = ingredient_index(recipe[counter]);
		
		// if the ingredient exists (ie. not an empty string)
		if (ing_row != NOT_FOUND && ing_row != change_ingredient_index)
		{
			// get the weight of every other related ingredient
			for (int col = 0; col < MAT_SIZE; col++)
			{
				// update the weight of each related ingredient
				weights[col] += AdjMat[ing_row][col];
			}
		}
	}
	
	// now we have an array that contains the weight of each ingredient
	// except we don't want to include the ingredients that are already in the recipe
	// get rid of the weights of the ingredients that are already in the recipe
	// also get rid of the ingredient we are subbing
	remove_weights(weights, recipe);
	weights[ingredient_index(to_change)] = INVALID_WEIGHT;
	max_index = get_index_of_max(weights);
	
	if (max_index != NOT_FOUND && weights[max_index] > 0)
	// there is a compatible sub
	{
